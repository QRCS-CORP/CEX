...
namespace Test
{
	...
	std::string TwofishTest::Run()
	{
		...
		try
		{
			...
			HexConverter::Decode(std::string("282BE7E4FA1FBDC29661286F1F310B7E"), output);
			// 128 key
			...
			// 192 key
			key.resize(24, 0);
			HexConverter::Decode(std::string("9AB71D7F280FF79F0D135BBD5FAB7E37"), output);
			CompareMonteCarlo(key, m_plainText, output);
			OnProgress(std::string("TwofishTest: Passed 10,000 round 192 bit key Monte Carlo encryption test.."));

			// 256 key
			key.resize(32, 0);
			HexConverter::Decode(std::string("04F2F36CA927AE506931DE8F78B2513C"), output);
			CompareMonteCarlo(key, m_plainText, output);
			OnProgress(std::string("TwofishTest: Passed 10,000 round 256 bit key Monte Carlo encryption test.."));

			// decrypt 10,000 rounds
			key.resize(16, 0);
			HexConverter::Decode(std::string("21D3F7F6724513946B72CFAE47DA2EED"), output);
			// 128 key
			...
			// 192 key
			key.resize(24, 0);
			HexConverter::Decode(std::string("B4582FA55072FCFEF538F39072F234A9"), output);
			CompareMonteCarlo(key, m_plainText, output, false);
			OnProgress(std::string("TwofishTest: Passed 10,000 round 192 bit key Monte Carlo decryption test.."));

			// 256 key
			key.resize(32, 0);
			HexConverter::Decode(std::string("BC7D078C4872063869DEAB891FB42761"), output);
			CompareMonteCarlo(key, m_plainText, output, false);
	...
	{
		HexConverter::Decode(std::string("00000000000000000000000000000000"), m_plainText);
	}
