...
namespace Test
{
	std::string ISCRsgTest::Run()
	{
		try
		{
			Initialize();
			CompareVector();
			OnProgress("Passed ISAAC vector tests..");
	...
	void ISCRsgTest::CompareVector()
	{
		std::vector<byte> seed(1020);
		CEX::Drbg::ISAAC gen;
		gen.Initialize(seed);
		gen.Reset();
		std::vector<byte> rnd(4, 0);

		for (unsigned int i = 0; i < m_expected.size(); i++)
		{
			gen.Generate(rnd);
			uint x = CEX::Utility::IntUtils::BytesToLe32(rnd, 0);
