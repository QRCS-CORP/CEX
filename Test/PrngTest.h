...
namespace Test
{
	...
	private:
		...
		static std::string ChiSquareG(Drbg::IDrbg* Generator, std::vector<byte> &Seed, size_t SampleSize = 1024000)
		{
			// converges slowly, needs 1mb or more
			std::vector<byte> rnd(SampleSize);
		...
		static std::string MeanValueG(Drbg::IDrbg* Generator, std::vector<byte> &Seed, size_t SampleSize = 102400)
		{
			// 100kb sample
			std::vector<byte> rnd(SampleSize);
			Generator->Initialize(Seed);
