...
namespace Test
{
    ...
    private:
		void CompareVector(Digest::IDigest* Digest, std::vector<std::vector<byte>> &Expected);
		void CompareDoFinal(Digest::IDigest* Digest);
		void CompareHMAC(Digest::IDigest* Digest, std::vector<std::vector<byte>> &Expected, std::vector<byte> &TruncExpected);
		void Initialize();
