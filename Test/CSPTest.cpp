...
namespace Test
{
	...
	const std::string CSPTest::FAILURE = "FAILURE! ";
	...
	void CSPTest::Evaluate(IProvider* Rng)
	{
		...
		{
			throw TestException(std::string("CSP"), std::string("Exception: Ordered runs test failure! -AE1"));
		}

		// succesive zeroes
		if (TestUtils::SuccesiveZeros(otp))
		{
			throw TestException(std::string("CSP"), std::string("Exception: Succesive zeroes test failure! -AE2"));
		}
	}
	...
	void CSPTest::Exception()
	{
		...
		try
		{
			...
			throw TestException(std::string("CSP"), std::string("Exception: Exception handling failure! -AE3"));
		}
	...
	void CSPTest::OnProgress(std::string Data)
	{
	...
	void CSPTest::Stress()
	{
		...
		for (i = 0; i < TEST_CYCLES; ++i)
		{
			...
			{
				throw TestException(std::string("CSP"), std::string("Stress: The generator has thrown an exception! -AS1"));
			}
		}
	}
}
