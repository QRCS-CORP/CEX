...
Skein512::Skein512(SkeinStateType InitializationType)
	:
	m_blockCipher(),
	m_bytesFilled(0),
...
size_t Skein512::Finalize(std::vector<byte> &Output, const size_t OutOffset)
{
	if (Output.size() - OutOffset < DIGEST_SIZE)
		throw CryptoDigestException("Skein512:Finalize", "The Output buffer is too short!");
...
void Skein512::Update(const std::vector<byte> &Input, size_t InOffset, size_t Length)
{
	if ((InOffset + Length) > Input.size())
		throw CryptoDigestException("Skein512:Update", "The Input buffer is too short!");
