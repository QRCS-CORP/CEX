...
public:
	...
	/// </summary>
	static const size_t RLWE_PUBPOLY_SIZE = ((14 * RLWE_N) / 8);

	/// <summary>
	/// The byte size of the compressed ciphertext polynomial
	/// </summary>
	static const size_t RLWE_POLYCOMPRESSEDBYTES = ((3 * RLWE_N) / 8);
	...
	/// </summary>
	static const size_t RLWE_PUBLICKEY_SIZE = RLWE_PUBPOLY_SIZE + RLWE_SEED_SIZE;

	/// <summary>
	/// The byte size of the private key polynomial
	/// </summary>
	static const size_t RLWE_PRIVATEKEY_SIZE = RLWE_N;

	/// <summary>
	/// The byte size of B's reply message to host A
	/// </summary>
	static const size_t RLWE_CIPHERTEXT_SIZE = RLWE_PUBPOLY_SIZE + RLWE_RECEIVED_SIZE; // RLWE_POLYCOMPRESSEDBYTES;
	...
	static void Encrypt(std::vector<byte> &Secret, std::vector<byte> &Send, const std::vector<byte> &Received, std::unique_ptr<Prng::IPrng> &Rng);
	...
	/// <param name="Rng">The random provider</param>
	/// <param name="Parallel">Run in parallel or sequential mode</param>
	static void Generate(std::vector<byte> &PublicKey, std::vector<ushort> &PrivateKey, std::unique_ptr<Prng::IPrng> &Rng);

	/*static void RLWEQ12289N1024::SelfTest()
	{
		std::vector<byte> msg1(RLWE_SEED_SIZE);
		std::vector<byte> msg2(RLWE_SEED_SIZE);
		std::vector<byte> ct(RLWE_CIPHERTEXT_SIZE);
		std::vector<byte> pk(RLWE_PUBLICKEY_SIZE);
		std::vector<byte> sk(RLWE_PRIVATEKEY_SIZE * 2);
		std::vector<byte> coin(RLWE_SEED_SIZE);

		Prng::IPrng* rng = new Prng::CSR();
		std::unique_ptr<Prng::IPrng> rngP(rng);

		Generate2(pk, sk, rngP);

		rng->GetBytes(coin);
		Encrypt2(ct, msg1, pk, coin);
...
private:
	...
	inline static Vector CalcK(Vector &V0, Vector &V1, Vector &X, int RLWE_Q)
	{
		const Vector NQ(RLWE_Q);
		const Vector N1(1);
		Vector xit;
		Vector tmpT;
		Vector tmpR;
	...
	template <typename Vector>
	inline static Vector Decode(Vector &X, const int RLWE_Q)
	{
		Vector tmpT;
		Vector tmpC;
		Vector tmpB;
	...
	inline static void FwdNTT(Array &A)
	{
		...
		for (i = 0; i < 10; i += 2)
		{
			...
			for (j = 0; j < dist; j++)
			{
				for (k = j, jt = 0; k < RLWE_N - 1; k += 2 * dist, jt++)
				{
	...
	inline static void GetNoise(ArrayA &R, ArrayB &Random, int RLWE_Q)
	{
		Vector tmpA;
		Vector tmpB;
		Vector tmpR;
...
#if defined(__AVX__) || defined(__AVX2__) || defined(__AVX512__)
		const Vector NQ(RLWE_Q);
		const size_t ULVLEN = Vector::size() / sizeof(uint);
		std::vector<uint> tmpU(ULVLEN);
#else
...
#else
			R[i] = tmpA + RLWE_Q - tmpB;
#endif
		}
	}

	template <typename Vector, typename ArrayA, typename ArrayB>
	inline static void HelpRec(ArrayA &C, const ArrayA &V, ArrayB &Random, int RLWE_Q)
	{
#if defined(__AVX__) || defined(__AVX2__) || defined(__AVX512__)
		const Vector NQ(RLWE_Q);
		const Vector N1(1);
		...
		std::vector<int> tmpC(ULVLEN * 4);

		for (size_t i = 0; i < V.size() / 4; i += ULVLEN)
		{
...
#	endif 
			...
			tmpK = CalcK<Vector>(v0[0], v1[0], N8 * t0 + N4 * tmpR, RLWE_Q);
			tmpK += CalcK<Vector>(v0[1], v1[1], N8 * t1 + N4 * tmpR, RLWE_Q);
			tmpK += CalcK<Vector>(v0[2], v1[2], N8 * t2 + N4 * tmpR, RLWE_Q);
			tmpK += CalcK<Vector>(v0[3], v1[3], N8 * t3 + N4 * tmpR, RLWE_Q);
			...
			c0.Store(tmpC, 0);
			c1.Store(tmpC, ULVLEN);
			c2.Store(tmpC, ULVLEN * 2);
			c3.Store(tmpC, ULVLEN * 3);

			for (uint j = static_cast<uint>(tmpC.size() - 1), k = 0; k < ULVLEN; --j, ++k)
			{
				C[i + k + 768] = static_cast<ushort>(tmpC[j]);
				C[i + k + 512] = static_cast<ushort>(tmpC[j - ULVLEN]);
				C[i + k + 256] = static_cast<ushort>(tmpC[j - (ULVLEN * 2)]);
				C[i + k] = static_cast<ushort>(tmpC[j - (ULVLEN * 3)]);
			}
		}
		...
		for (size_t i = 0; i < V.size() / 4; i++)
		{
			...
			x = (8 * V[i]) + (4 * rbit);
			k = CalcK<int>(v0[0], v1[0], x, RLWE_Q);
			x = (8 * V[256 + i]) + (4 * rbit);
			k += CalcK<int>(v0[1], v1[1], x, RLWE_Q);
			x = (8 * V[512 + i]) + (4 * rbit);
			k += CalcK<int>(v0[2], v1[2], x, RLWE_Q);
			x = (8 * V[768 + i]) + (4 * rbit);
			k += CalcK<int>(v0[3], v1[3], x, RLWE_Q);
			k = ((2 * RLWE_Q) - 1 - k) >> 31;
	...
	template <typename Vector>
	inline static Vector LdDecode(Vector &X0, Vector &X1, Vector &X2, Vector &X3, const int RLWE_Q)
	{
		Vector tmpT;

		tmpT = Decode<Vector>(X0, RLWE_Q);
		tmpT += Decode<Vector>(X1, RLWE_Q);
		tmpT += Decode<Vector>(X2, RLWE_Q);
		tmpT += Decode<Vector>(X3, RLWE_Q);
		tmpT -= Vector(8) * Vector(RLWE_Q);
		tmpT >>= 31;
		tmpT &= Vector(1);
	...
	{
#if defined(__AVX512__)
		GetNoise<Numeric::UInt512, ArrayA, ArrayB>(R, Random, RLWE_Q);
#elif defined(__AVX2__)
		GetNoise<Numeric::UInt256, ArrayA, ArrayB>(R, Random, RLWE_Q);
#elif defined(__AVX__)
		GetNoise<Numeric::UInt128, ArrayA, ArrayB>(R, Random, RLWE_Q);
#else
		GetNoise<uint, ArrayA, ArrayB>(R, Random, RLWE_Q);
#endif
	}
	...
	template <typename Vector, typename ArrayA, typename ArrayB>
	inline static void Rec(ArrayA &Key, const ArrayB &V, const ArrayB &C, const int RLWE_Q)
	{
		...
		for (size_t i = 0; i < V.size() / 4; i += ULVLEN)
		{
			...
			Vector K = LdDecode<Vector>(tmpV[0], tmpV[1], tmpV[2], tmpV[3], RLWE_Q);

			K.Store(tmpK, 0);

			for (size_t j = ULVLEN, k = 0; j > 0; --j, ++k)
			{
		...
		{
			tmp[0] = (16 * RLWE_Q) + (8 * static_cast<int>(V[0 + i])) - (RLWE_Q * ((2 * C[0 + i]) + C[768 + i]));
			tmp[1] = (16 * RLWE_Q) + (8 * static_cast<int>(V[256 + i])) - (RLWE_Q * ((2 * C[256 + i]) + C[768 + i]));
			tmp[2] = (16 * RLWE_Q) + (8 * static_cast<int>(V[512 + i])) - (RLWE_Q * ((2 * C[512 + i]) + C[768 + i]));
			tmp[3] = (16 * RLWE_Q) + (8 * static_cast<int>(V[768 + i])) - (RLWE_Q * C[768 + i]);
			Key[i >> 3] |= LdDecode<int>(tmp[0], tmp[1], tmp[2], tmp[3], RLWE_Q) << (i & 7);
		}
#endif
	}
	...
	static void DecodeA(std::array<ushort, RLWE_N> &PubKey, std::vector<byte> &Seed, const std::vector<byte> &R);
	static void DecodeB(std::array<ushort, RLWE_N> &B, std::array<ushort, RLWE_N> &C, const std::vector<byte> &R);
	static void EncodeA(std::vector<byte> &R, const std::array<ushort, RLWE_N> &PubKey, const std::vector<byte> &Seed);
	static void EncodeB(std::vector<byte> &R, const std::array<ushort, RLWE_N> &B, const std::array<ushort, RLWE_N> &C);
	...
	static void RecHelper(std::array<ushort, RLWE_N> &C, const std::array<ushort, RLWE_N> &V, std::vector<byte> &Random);
	static void Reconcile(std::vector<byte> &Key, const std::array<ushort, RLWE_N> &V, const std::array<ushort, RLWE_N> &C);
	static void ToBytes(std::vector<byte> &R, const std::array<ushort, RLWE_N> &Poly);
};
