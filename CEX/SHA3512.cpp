...
class SHA3512::SHA3512State
{
public:

	std::array<uint64_t, 25> H = { 0 };
...
SHA3512::SHA3512(bool Parallel)
	:
	m_dgtState(Parallel ? 
		DEF_PRLDEGREE : 
		1),
	m_msgBuffer(Parallel ? 
		DEF_PRLDEGREE * Keccak::KECCAK512_RATE_SIZE : 
	...
	m_treeParams(Parallel ? 
		KeccakParams(Keccak::KECCAK512_DIGEST_SIZE, static_cast<uint8_t>(Keccak::KECCAK512_RATE_SIZE), static_cast<uint8_t>(m_parallelProfile.ParallelMaxDegree())) :
		KeccakParams(Keccak::KECCAK512_DIGEST_SIZE, 0x00, 0x00))
{
	Reset();
}

SHA3512::SHA3512(KeccakParams &Params)
	:
	m_dgtState(Params.FanOut() != 0 && Params.FanOut() <= MAX_PRLDEGREE ? 
		Params.FanOut() :
		throw CryptoDigestException(DigestConvert::ToName(Digests::SHA3512), std::string("Constructor"), std::string("The FanOut parameter can not be zero or exceed the maximum of 64!"), ErrorCodes::IllegalOperation)),
...
size_t SHA3512::BlockSize() 
{ 
	return Keccak::KECCAK512_RATE_SIZE; 
}

size_t SHA3512::DigestSize() 
{ 
	return Keccak::KECCAK512_DIGEST_SIZE; 
}

const Digests SHA3512::Enumeral() 
{
	return Digests::SHA3512;
}

const bool SHA3512::IsParallel() 
{
	return m_parallelProfile.IsParallel(); 
}

const std::string SHA3512::Name() 
{ 
	std::string name;
...
const size_t SHA3512::ParallelBlockSize() 
{ 
	return m_parallelProfile.ParallelBlockSize(); 
}

ParallelOptions &SHA3512::ParallelProfile() 
{ 
...
void SHA3512::Compute(const std::vector<uint8_t> &Input, std::vector<uint8_t> &Output)
{
...
void SHA3512::Finalize(std::vector<uint8_t> &Output, size_t OutOffset)
{
...
void SHA3512::ParallelMaxDegree(size_t Degree)
{
...
void SHA3512::Update(uint8_t Input)
{
	std::vector<uint8_t> one(1, Input);
	Update(one, 0, 1);
}

void SHA3512::Update(uint32_t Input)
{
...
void SHA3512::Update(uint64_t Input)
{
...
void SHA3512::Update(const std::vector<uint8_t> &Input, size_t InOffset, size_t Length)
{
...
void SHA3512::Permute(std::array<uint64_t, 25> & State)
{
...
void SHA3512::HashFinal(std::vector<uint8_t> &Input, size_t InOffset, size_t Length, SHA3512State &State)
{
	Keccak::Absorb(Input, InOffset, Length, Keccak::KECCAK512_RATE_SIZE, Keccak::KECCAK_SHA3_DOMAIN, State.H);
	Permute(State.H);
}

void SHA3512::ProcessLeaf(const std::vector<uint8_t> &Input, size_t InOffset, SHA3512State &State, uint64_t Length)
{
	do
	{
