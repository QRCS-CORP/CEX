...
const std::array<ushort, 256> FFTQ7681N256::Zetas =
{
...
const std::array<ushort, 128> FFTQ7681N256::OmegasInvMontgomery =
{
...
const std::array<ushort, 256> FFTQ7681N256::PsisInvMontgomery =
{
...
void FFTQ7681N256::Decrypt(std::vector<byte> &Secret, const std::vector<byte> &CipherText, const std::vector<byte> &PrivateKey, uint K)
{
	std::vector<std::array<ushort, N>> bp(K);
	std::vector<std::array<ushort, N>> skpv(K);
	std::array<ushort, N> mp;
...
void FFTQ7681N256::Encrypt(std::vector<byte> &CipherText, const std::vector<byte> &Message, const std::vector<byte> &PublicKey, std::unique_ptr<Prng::IPrng> &Rng, uint K)
{
	std::vector<std::vector<std::array<ushort, N>>> at(K, std::vector<std::array<ushort, N>>(K));
	std::vector<byte> seed(SEED_SIZE);
	std::vector<std::array<ushort, N>> bp(K);
	std::vector<std::array<ushort, N>> ep(K);
	std::vector<std::array<ushort, N>> pkpv(K);
	std::vector<std::array<ushort, N>> sp(K);
	std::array<ushort, N> epp;
	std::array<ushort, N> k;
	std::array<ushort, N> v;
	size_t i;
	size_t eta;
...
void FFTQ7681N256::Generate(std::vector<byte> &PublicKey, std::vector<byte> &PrivateKey, std::unique_ptr<Prng::IPrng> &Rng, uint K)
{
	std::vector<std::vector<std::array<ushort, N>>> a(K, std::vector<std::array<ushort, N>>(K));
	std::vector<std::array<ushort, N>> e(K);
	std::vector<std::array<ushort, N>> pkpv(K);
	std::vector<std::array<ushort, N>> skpv(K);
	std::vector<byte> noise(SEED_SIZE);
	std::vector<byte> seed(SEED_SIZE);
	size_t eta;
	size_t i;
	...
	for (i = 0; i < ((K * PUBPOLY_SIZE) + SEED_SIZE); i++)
	{
		PrivateKey[i + (K * PRIPOLY_SIZE)] = PublicKey[i];
	}
}

//~~~Static~~~//

void FFTQ7681N256::GenerateMatrix(std::vector<std::vector<std::array<ushort, N>>> &A, const std::vector<byte> &Seed, bool Transposed)
{
	byte i;
	byte j;
	ushort val;
...
void FFTQ7681N256::GetNoise(std::array<ushort, N> &R, size_t Eta, std::unique_ptr<Prng::IPrng> &Rng)
{
	std::vector<byte> buf((Eta * N) / 4);
	Rng->GetBytes(buf);

	Cbd(R, buf, Eta);
}

void FFTQ7681N256::InvNTT(std::array<ushort, N> &P)
{
	// TODO: vectorize
	ushort level;
	ushort tmp;
	...
	for (level = 0; level < 8; level++)
	{
		for (start = 0; start < (1U << level); start++)
		{
			...
			for (j = start; j < N - 1; j += 2 * (1U << level))
			{
				...
				t = static_cast<uint>(W) * (tmp + ((4 * Q) - P[tpos]));
...
void FFTQ7681N256::FwdNTT(std::array<ushort, N> &P)
{
	// TODO: vectorize
	short level;
	ushort start;
	...
	{
		for (start = 0; start < N; start = j + (1U << level))
		{
			zeta = Zetas[k];
			++k;
			...
			for (j = start; j < start + (1U << level); ++j)
			{
				...
				P[tpos] = BarrettReduce(P[j] + ((4 * Q) - t));
...
void FFTQ7681N256::Cbd(std::array<ushort, N> &R, const std::vector<byte> &Buffer, size_t Eta)
{
	// TODO: vectorize
	size_t i;
	size_t j;
...
void FFTQ7681N256::PackCiphertext(std::vector<byte> &R, const std::vector<std::array<ushort, N>> &B, const std::array<ushort, N> &V)
{
	PolyVecCompress(R, B);
	PolyCompress(R, (B.size() * PUBPOLY_SIZE), V);
}

void FFTQ7681N256::PackPublicKey(std::vector<byte> &R, const std::vector<std::array<ushort, N>> &Pk, const std::vector<byte> &Seed)
{
	size_t i;
	...
	{
		R[i + (Pk.size() * PUBPOLY_SIZE)] = Seed[i];
	}
}

void FFTQ7681N256::PackSecretKey(std::vector<byte> &R, const std::vector<std::array<ushort, N>> &Sk)
{
	PolyVecToBytes(R, Sk);
}

void FFTQ7681N256::PolyAdd(std::array<ushort, N> &R, const std::array<ushort, N> &A, const std::array<ushort, N> &B)
{
	size_t i;
...
void FFTQ7681N256::PolyCompress(std::vector<byte> &R, size_t Offset, const std::array<ushort, N> &A)
{
	std::array<uint, 8> t;
	size_t i;
	size_t j;
...
void FFTQ7681N256::PolyDecompress(std::array<ushort, N> &R, const std::vector<byte> &A, size_t Offset)
{
	size_t i;
	size_t pos;
	...
	{
		R[i] = ((((A[pos] & 7) * Q) + 4) >> 3);
		R[i + 1] = (((((A[pos] >> 3) & 7) * Q) + 4) >> 3);
		R[i + 2] = (((((A[pos] >> 6) | ((A[pos + 1] << 2) & 4)) * Q) + 4) >> 3);
		R[i + 3] = (((((A[pos + 1] >> 1) & 7) * Q) + 4) >> 3);
		R[i + 4] = (((((A[pos + 1] >> 4) & 7) * Q) + 4) >> 3);
		R[i + 5] = (((((A[pos + 1] >> 7) | ((A[pos + 2] << 1) & 6)) * Q) + 4) >> 3);
		R[i + 6] = (((((A[pos + 2] >> 2) & 7) * Q) + 4) >> 3);
		R[i + 7] = (((((A[pos + 2] >> 5)) * Q) + 4) >> 3);
		pos += 3;
	}
}

void FFTQ7681N256::PolyFrombytes(std::array<ushort, N> &R, const std::vector<byte> &A, size_t Offset)
{
	size_t i;
...
void FFTQ7681N256::PolySub(std::array<ushort, N> &R, const std::array<ushort, N> &A, const std::array<ushort, N> &B)
{
	size_t i;

	for (i = 0; i < N; i++)
	{
		R[i] = BarrettReduce(A[i] + ((3 * Q) - B[i]));
	}
}

void FFTQ7681N256::PolyToBytes(std::vector<byte> &R, size_t Offset, const std::array<ushort, N> &A)
{
	ushort t[8];
	size_t i;
	size_t j;
...
void FFTQ7681N256::PolyToMsg(std::vector<byte> &Message, const std::array<ushort, N> &A)
{
	size_t i;
	size_t j;
	ushort t;
	...
	for (i = 0; i < SEED_SIZE; i++)
	{
		...
		for (j = 0; j < 8; j++)
		{
			t = ((((Freeze(A[(8 * i) + j]) << 1) + Q / 2) / Q) & 1);
			Message[i] |= (t << j);
		}
	}
}

void FFTQ7681N256::PolyVecAdd(std::vector<std::array<ushort, N>> &R, const std::vector<std::array<ushort, N>> &A, const std::vector<std::array<ushort, N>> &B)
{
	size_t i;
...
void FFTQ7681N256::PolyVecCompress(std::vector<byte> &R, size_t Offset, const std::vector<std::array<ushort, N>> &A)
{
	ushort t[8];
	size_t i;
	size_t j;
	...
	for (i = 0; i < A.size(); i++)
	{
		for (j = 0; j < N / 8; j++)
		{
			for (k = 0; k < 8; k++)
			{
				t[k] = (((static_cast<uint>(Freeze(A[i][(8 * j) + k])) << 11) + Q / 2) / Q) & 0x7FF;
			}
...
void FFTQ7681N256::PolyFromMessage(std::array<ushort, N> &R, const std::vector<byte> &Message)
{
	size_t i;
	size_t j;
	ushort mask;
...
void FFTQ7681N256::PolyVecCompress(std::vector<byte> &R, const std::vector<std::array<ushort, N>> &A)
{
	ushort t[8];
	size_t i;
	size_t j;
...
void FFTQ7681N256::PolyVecDecompress(std::vector<std::array<ushort, N>> &R, const std::vector<byte> &A)
{
	size_t i;
	size_t j;
	size_t pos;
	...
	for (i = 0; i < R.size(); i++)
	{
		for (j = 0; j < N / 8; j++)
		{
			R[i][8 * j] = ((((A[pos + (11 * j)] | ((static_cast<uint>(A[pos + (11 * j) + 1]) & 0x07) << 8)) * Q) + 1024) >> 11);
			R[i][(8 * j) + 1] = (((((A[pos + (11 * j) + 1] >> 3) | ((static_cast<uint>(A[pos + (11 * j) + 2]) & 0x3F) << 5)) * Q) + 1024) >> 11);
			R[i][(8 * j) + 2] = (((((A[pos + (11 * j) + 2] >> 6) | ((static_cast<uint>(A[pos + (11 * j) + 3]) & 0xFF) << 2) | ((static_cast<uint>(A[pos + (11 * j) + 4]) & 0x01) << 10)) * Q) + 1024) >> 11);
			R[i][(8 * j) + 3] = (((((A[pos + (11 * j) + 4] >> 1) | ((static_cast<uint>(A[pos + (11 * j) + 5]) & 0x0F) << 7)) * Q) + 1024) >> 11);
			R[i][(8 * j) + 4] = (((((A[pos + (11 * j) + 5] >> 4) | ((static_cast<uint>(A[pos + (11 * j) + 6]) & 0x7F) << 4)) * Q) + 1024) >> 11);
			R[i][(8 * j) + 5] = (((((A[pos + (11 * j) + 6] >> 7) | ((static_cast<uint>(A[pos + (11 * j) + 7]) & 0xFF) << 1) | ((static_cast<uint>(A[pos + (11 * j) + 8]) & 0x03) << 9)) * Q) + 1024) >> 11);
			R[i][(8 * j) + 6] = (((((A[pos + (11 * j) + 8] >> 2) | ((static_cast<uint>(A[pos + (11 * j) + 9]) & 0x1F) << 6)) * Q) + 1024) >> 11);
			R[i][(8 * j) + 7] = (((((A[pos + (11 * j) + 9] >> 5) | ((static_cast<uint>(A[pos + (11 * j) + 10]) & 0xFF) << 3)) * Q) + 1024) >> 11);
		}
		pos += PUBPOLY_SIZE;
	}
}

void FFTQ7681N256::PolyVecFrombytes(std::vector<std::array<ushort, N>> &R, const std::vector<byte> &A)
{
	size_t i;

	for (i = 0; i < R.size(); i++)
	{
		PolyFrombytes(R[i], A, (i * PRIPOLY_SIZE));
	}
}

void FFTQ7681N256::PolyVecInvNTT(std::vector<std::array<ushort, N>> &R)
{
	size_t i;
...
void FFTQ7681N256::PolyVecNTT(std::vector<std::array<ushort, N>> &R)
{
	size_t i;
...
void FFTQ7681N256::PolyVecPointwiseAcc(std::array<ushort, N> &R, const std::vector<std::array<ushort, N>> &A, const std::vector<std::array<ushort, N>> &B)
{
	// TODO: vectorize
	size_t i;
	size_t j;
...
void FFTQ7681N256::PolyVecToBytes(std::vector<byte> &R, const std::vector<std::array<ushort, N>> &A)
{
	size_t i;

	for (i = 0; i < A.size(); i++)
	{
		PolyToBytes(R, (i * PRIPOLY_SIZE), A[i]);
	}
}

void FFTQ7681N256::UnpackCiphertext(std::vector<std::array<ushort, N>> &B, std::array<ushort, N> &V, const std::vector<byte> &C)
{
	PolyVecDecompress(B, C);
	PolyDecompress(V, C, B.size() * PUBPOLY_SIZE);

}

void FFTQ7681N256::UnpackPublicKey(std::vector<std::array<ushort, N>> &Pk, std::vector<byte> &Seed, const std::vector<byte> &PackedPk)
{
	size_t i;
	...
	for (i = 0; i < SEED_SIZE; i++)
	{
		Seed[i] = PackedPk[i + (Pk.size() * PUBPOLY_SIZE)];
	}
}

void FFTQ7681N256::UnpackSecretKey(std::vector<std::array<ushort, N>> &Sk, const std::vector<byte> &PackedSk)
{
