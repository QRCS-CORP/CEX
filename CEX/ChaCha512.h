...
/// <item><description>The nonce value is optional; if authentication is enabled, the nonce and key initialize cSHAKE(k,n), if absent SHAKE(k) is used to expand the key.</description></item>
...
/// <item><description>Permutation rounds are set at 80 if CEX_CHACHA512_STRONG is defined in CexConfig, otherwise the rounds revert to 40.</description></item>
...
public:
	...
	/// If encryption and authentication are set to true, the MAC code can be appended to the ciphertext array using the Finalize(Output, Offset, Length) function.
	/// If the Nonce value in the symmetric key structure is non-zero and authentication is enabled, 
...
private:
	...
	void Expand(const std::vector<byte> &Key);
