...
ACS::ACS(BlockCiphers CipherType, StreamAuthenticators AuthenticatorType)
	:
	m_acsState(new AcsState(AuthenticatorType)),
...
{
	StreamCiphers tmpn;
	StreamAuthenticators auth = IsAuthenticator() ? static_cast<StreamAuthenticators>(m_macAuthenticator->Enumeral()) : StreamAuthenticators::None;

	switch (auth)
	{
		case StreamAuthenticators::HMACSHA256:
		{
			tmpn = StreamCiphers::ACS256H;
			break;
		}
		case StreamAuthenticators::HMACSHA512:
		{
			tmpn = StreamCiphers::ACS512H;
			break;
		}
		case StreamAuthenticators::KMAC256:
		{
			tmpn = StreamCiphers::ACS256S;
			break;
		}
		case StreamAuthenticators::KMAC512:
		{
			tmpn = StreamCiphers::ACS512S;
			break;
		}
		default:
		{
			tmpn = StreamCiphers::ACS;
		}
	}
...
const std::string ACS::Name()
{
	...
	if (m_macAuthenticator != nullptr)
	{
		name += std::string("-") + Enumeration::StreamCipherConvert::ToName(Enumeral());
	}
