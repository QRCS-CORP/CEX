...
Threefish1024::Threefish1024(StreamAuthenticators AuthenticatorType)
	:
...
void Threefish1024::Authenticator(StreamAuthenticators AuthenticatorType)
{
	if (m_macAuthenticator != nullptr)
	{
		m_macAuthenticator.reset(nullptr);
...
void Threefish1024::Initialize(bool Encryption, ISymmetricKey &Parameters)
{
	...
	if (m_authenticatorType == StreamAuthenticators::None)
	{
		...
		m_cipherState->K[8] = IntegerTools::LeBytesTo64(Parameters.Key(), 64);
		m_cipherState->K[9] = IntegerTools::LeBytesTo64(Parameters.Key(), 72);
		m_cipherState->K[10] = IntegerTools::LeBytesTo64(Parameters.Key(), 80);
		m_cipherState->K[11] = IntegerTools::LeBytesTo64(Parameters.Key(), 88);
		m_cipherState->K[12] = IntegerTools::LeBytesTo64(Parameters.Key(), 96);
		m_cipherState->K[13] = IntegerTools::LeBytesTo64(Parameters.Key(), 104);
		m_cipherState->K[14] = IntegerTools::LeBytesTo64(Parameters.Key(), 112);
		m_cipherState->K[15] = IntegerTools::LeBytesTo64(Parameters.Key(), 120);
	}
	else
	{
...
void Threefish1024::Finalize(std::vector<byte> &Output, const size_t OutOffset, const size_t Length)
{
	...
	// customization string is: mac counter + algorithm formal name
	IntegerTools::Le64ToBytes(m_macCounter, m_shakeCustom, 0);
