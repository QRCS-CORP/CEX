...
void OCB::DecryptBlock(const std::vector<byte> &Input, const size_t InOffset, std::vector<byte> &Output, const size_t OutOffset)
{
	...
	IntUtils::XORBLK(hash, 0, m_mainOffset, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
	IntUtils::XORBLK(m_mainOffset, 0, Output, OutOffset, BLOCK_SIZE, m_parallelProfile.HasSimd128());
	m_blockCipher->Transform(Output, OutOffset, Output, OutOffset);
	IntUtils::XORBLK(m_mainOffset, 0, Output, OutOffset, BLOCK_SIZE, m_parallelProfile.HasSimd128());
	IntUtils::XORBLK(Output, OutOffset, m_checkSum, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
}
...
void OCB::SetAssociatedData(const std::vector<byte> &Input, const size_t Offset, const size_t Length)
{
	...
	while (blkLen >= BLOCK_SIZE)
	{
		...
		IntUtils::XORBLK(offset, 0, offsetHash, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
		IntUtils::XORBLK(offsetHash, 0, tmp, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
		m_hashCipher->Transform(tmp, 0, tmp, 0);
		IntUtils::XORBLK(tmp, 0, m_aadData, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
		blkOff += BLOCK_SIZE;
		blkLen -= BLOCK_SIZE;
	}
	...
	if (blkLen != 0)
	{
		...
		IntUtils::XORBLK(m_listAsterisk, 0, offsetHash, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
		IntUtils::XORBLK(offsetHash, 0, tmp, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
		m_hashCipher->Transform(tmp, 0, tmp, 0);
		IntUtils::XORBLK(tmp, 0, m_aadData, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
	}
...
{
	IntUtils::XORBLK(m_mainOffset, 0, m_checkSum, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
	IntUtils::XORBLK(m_listDollar, 0, m_checkSum, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
	m_hashCipher->Transform(m_checkSum, 0, m_checkSum, 0);
	IntUtils::XORBLK(m_aadData, 0, m_checkSum, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
...
void OCB::ProcessSegment(const std::vector<byte> &Input, size_t InOffset, std::vector<byte> &Output, size_t OutOffset, size_t Length)
{
	...
	if (m_parallelProfile.HasSimd256() && Length >= AVXBLK)
	{
		...
		IntUtils::XORBLK(Input, InOffset, Output, OutOffset, PBKALN, m_parallelProfile.HasSimd128());
...
void OCB::ParallelDecrypt(const std::vector<byte> &Input, size_t InOffset, std::vector<byte> &Output, size_t OutOffset, size_t Length)
{
	...
	if (Length != 0)
	{
		...
		{
			IntUtils::XORBLK(offsetChain, chainPos, Output, OutOffset, BLOCK_SIZE, m_parallelProfile.HasSimd128());
			m_blockCipher->Transform(Output, OutOffset, Output, OutOffset);
			IntUtils::XORBLK(offsetChain, chainPos, Output, OutOffset, BLOCK_SIZE, m_parallelProfile.HasSimd128());
	...
	// update the checksum
	for (size_t i = 0; i < BLKCNT; ++i)
		IntUtils::XORBLK(Output, OUTOFF + (i * BLOCK_SIZE), m_checkSum, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
}
...
void OCB::ParallelEncrypt(const std::vector<byte> &Input, size_t InOffset, std::vector<byte> &Output, size_t OutOffset, size_t Length)
{
	...
	for (size_t i = 0; i < BLKCNT; ++i)
		IntUtils::XORBLK(Output, OutOffset + (i * BLOCK_SIZE), m_checkSum, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
...
void OCB::ProcessPartial(const std::vector<byte> &Input, const size_t InOffset, std::vector<byte> &Output, const size_t OutOffset, size_t Length)
{
	if (m_isEncryption)
	{
		...
		IntUtils::XORBLK(m_listAsterisk, 0, m_mainOffset, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());

		std::vector<byte> pad(BLOCK_SIZE);
		m_hashCipher->Transform(m_mainOffset, 0, pad, 0);
		IntUtils::XORBLK(pad, 0, Output, OutOffset, BLOCK_SIZE, m_parallelProfile.HasSimd128());
	}
	else
	{
		memcpy(&Output[OutOffset], &Input[InOffset], Length);
		...
		IntUtils::XORBLK(tmp, 0, m_checkSum, 0, BLOCK_SIZE, m_parallelProfile.HasSimd128());
	}
}
