...
bool NTRUSQ4621P653::Decapsulate(std::vector<byte> &Secret, const std::vector<byte> &CipherText, const std::vector<byte> &PrivateKey)
{
	// k = Decap(c,sk)
...
void NTRUSQ4621P653::Encapsulate(std::vector<byte> &Secret, std::vector<byte> &CipherText, const std::vector<byte> &PublicKey, std::unique_ptr<Prng::IPrng> &Rng)
{
	// c,k = Encap(pk)
...
void NTRUSQ4621P653::Generate(std::vector<byte> &PublicKey, std::vector<byte> &PrivateKey, std::unique_ptr<Prng::IPrng> &Rng)
{
...
void NTRUSQ4621P653::Hash(std::vector<byte> &Output, size_t OutOffset, int B, const std::vector<byte> &Input)
{
...
void NTRUSQ4621P653::HashConfirm(std::vector<byte> &H, size_t HOffset, const std::vector<byte> &R, const std::vector<byte> &Pk, const std::vector<byte> &Cache) // is pk supposed to be hashed here?
{
...
void NTRUSQ4621P653::HashSession(std::vector<byte> &K, int B, const std::vector<byte> &Y, const std::vector<byte> &Z)
{
	// k = HashSession(b,y,z)
...
void NTRUSQ4621P653::Hide(std::vector<byte> &C, std::vector<byte> &REnc, const std::vector<int8_t> &R, const std::vector<byte> &Pk, const std::vector<byte> &Cache)
{
...
void NTRUSQ4621P653::KeyGen(std::vector<int16_t> &H, std::vector<int8_t> &F, std::vector<int8_t> &GInv, std::unique_ptr<Prng::IPrng> &Rng)
{
	// h,(f,ginv) = KeyGen()
...
void NTRUSQ4621P653::ShortFromList(std::vector<int8_t> &Output, const std::vector<uint> &Input)
{
...
void NTRUSQ4621P653::ShortRandom(std::vector<int8_t> &Output, std::unique_ptr<Prng::IPrng> &Rng)
{
...
void NTRUSQ4621P653::SmallRandom(std::vector<int8_t> &Output, std::unique_ptr<Prng::IPrng> &Rng)
{
	size_t i;
...
uint NTRUSQ4621P653::URandom32(std::unique_ptr<Prng::IPrng> &Rng)
{
...
void NTRUSQ4621P653::ZKeyGen(std::vector<byte> &Pk, std::vector<byte> &Sk, std::unique_ptr<Prng::IPrng> &Rng)
{
	// pk,sk = ZKeyGen() 
...
void NTRUSQ4621P653::ZDecrypt(std::vector<int8_t> &R, const std::vector<byte> &C, const std::vector<byte> &Sk)
{
	// r = ZDecrypt(C,sk)
...
void NTRUSQ4621P653::ZEncrypt(std::vector<byte> &C, const std::vector<int8_t> &R, const std::vector<byte> &Pk)
{
	// C = ZEncrypt(r,pk)
