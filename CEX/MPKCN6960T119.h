...
public:
	...
	/// The dimension: N
	/// </summary>
	static const uint32_t MPKC_N = 6960;

	/// <summary>
	/// The error correction capability of the code: T
	/// </summary>
	static const uint32_t MPKC_T = 119;

	/// <summary>
	/// The uint8_t size of the CCA private key polynomial
	/// </summary>
	static const size_t PRIVATEKEY_SIZE = 13948;
	...
	static bool Encapsulate(const std::vector<uint8_t> &PublicKey, std::vector<uint8_t> &CipherText, std::vector<uint8_t> &SharedSecret, std::unique_ptr<IPrng>& Rng);
...
private:
	...
	static int32_t CheckCPadding(const uint8_t* C);
	static int32_t CheckPkPadding(const uint8_t* Pk);
