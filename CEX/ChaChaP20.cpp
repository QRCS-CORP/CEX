...
const std::vector<byte> ChaChaP20::SIGMA_INFO = { 0x65, 0x78, 0x70, 0x61, 0x6E, 0x64, 0x20, 0x33, 0x32, 0x2D, 0x62, 0x79, 0x74, 0x65, 0x20, 0x6B };
...
ChaChaP20::ChaChaP20(SecureVector<byte> &State)
	:
...
const std::vector<byte> ChaChaP20::Nonce()
{
...
const std::vector<byte> ChaChaP20::Tag()
{
...
const void ChaChaP20::Tag(SecureVector<byte> &Output)
{
	if (m_csx256State->MacTag.size() == 0 || IsAuthenticator() == false)
	{
		throw CryptoSymmetricException(std::string("ChaChaP20"), std::string("Tag"), std::string("The cipher is not initialized for authentication or has not run!"), ErrorCodes::NotInitialized);
...
SecureVector<byte> ChaChaP20::Serialize()
{
...
void ChaChaP20::SetAssociatedData(const std::vector<byte> &Input, size_t Offset, size_t Length)
{
...
void ChaChaP20::Transform(const std::vector<byte> &Input, size_t InOffset, std::vector<byte> &Output, size_t OutOffset, size_t Length)
{
...
void ChaChaP20::Generate(std::unique_ptr<CSX256State> &State, std::array<uint, 2> &Counter, std::vector<byte> &Output, size_t OutOffset, size_t Length)
{
	size_t ctr;
...
void ChaChaP20::Load(const SecureVector<byte> &Key, const SecureVector<byte> &Nonce, const SecureVector<byte> &Code)
{
...
void ChaChaP20::Process(const std::vector<byte> &Input, size_t InOffset, std::vector<byte> &Output, size_t OutOffset, size_t Length)
{
