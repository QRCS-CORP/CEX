...
SHA2256::SHA2256(bool Parallel)
	:
	m_dgtState(Parallel ? 
		DEF_PRLDEGREE : 
		1),
	m_msgBuffer(Parallel ? 
		DEF_PRLDEGREE * SHA2::SHA2256_RATE_SIZE : 
		SHA2::SHA2256_RATE_SIZE),
	m_msgLength(0),
...
SHA2256::SHA2256(SHA2Params &Params)
	:
	m_dgtState(Params.FanOut() != 0 && Params.FanOut() <= MAX_PRLDEGREE ? 
		Params.FanOut() :
		throw CryptoDigestException(DigestConvert::ToName(Digests::SHA2256), std::string("Constructor"), std::string("The FanOut parameter can not be zero or exceed the maximum of 64!"), ErrorCodes::IllegalOperation)),
	m_msgBuffer(Params.FanOut() * SHA2::SHA2256_RATE_SIZE),
	m_msgLength(0),
	m_parallelProfile(SHA2::SHA2256_RATE_SIZE, static_cast<bool>(Params.FanOut() > 1), false, STATE_PRECACHED, false, Params.FanOut()),
	m_treeParams(Params)
{
	Reset();
}
...
size_t SHA2256::BlockSize() 
{ 
	return SHA2::SHA2256_RATE_SIZE; 
}

size_t SHA2256::DigestSize() 
{ 
	return SHA2::SHA2256_DIGEST_SIZE;
}

const Digests SHA2256::Enumeral() 
{
	return Digests::SHA2256; 
}

const bool SHA2256::IsParallel()
{ 
	return m_parallelProfile.IsParallel(); 
}

const std::string SHA2256::Name()
{ 
	std::string name;
...
const size_t SHA2256::ParallelBlockSize()
{ 
	return m_parallelProfile.ParallelBlockSize(); 
}

ParallelOptions &SHA2256::ParallelProfile() 
{ 
...
void SHA2256::Finalize(std::vector<byte> &Output, size_t OutOffset)
{
	...
	if (m_parallelProfile.IsParallel())
	{
		...
		if (m_msgLength != 0)
		{
			...
			while (m_msgLength != 0)
			{
				const size_t MSGRMD = (m_msgLength >= SHA2::SHA2256_RATE_SIZE) ? SHA2::SHA2256_RATE_SIZE : m_msgLength;
				HashFinal(m_msgBuffer, bctr * SHA2::SHA2256_RATE_SIZE, MSGRMD, m_dgtState[bctr]);
				m_msgLength -= MSGRMD;
				++bctr;
			}
		}
		...
		{
			IntegerTools::BeUL256ToBlock(m_dgtState[i].H, 0, m_msgBuffer, i * SHA2::SHA2256_RATE_SIZE);
			m_msgLength += SHA2::SHA2256_DIGEST_SIZE;
		}

		// compress full blocks
		boff = 0;
		if (m_msgLength > SHA2::SHA2256_RATE_SIZE)
		{
			const size_t BLKRMD = m_msgLength - (m_msgLength % SHA2::SHA2256_RATE_SIZE);

			for (i = 0; i < BLKRMD / SHA2::SHA2256_RATE_SIZE; ++i)
			{
				Permute(m_msgBuffer, i * SHA2::SHA2256_RATE_SIZE, proot);
			}
...
void SHA2256::ParallelMaxDegree(size_t Degree)
{
...
void SHA2256::Reset()
{
	...
	m_msgBuffer.clear();
	m_msgBuffer.resize(m_parallelProfile.IsParallel() ? m_parallelProfile.ParallelMaxDegree() * SHA2::SHA2256_RATE_SIZE : SHA2::SHA2256_RATE_SIZE);
	m_msgLength = 0;
	...
	if (Length != 0)
	{
		if (m_parallelProfile.IsParallel())
		{
			if (m_msgLength != 0 && Length + m_msgLength >= m_msgBuffer.size())
			{
				...
				{
					Permute(m_msgBuffer, i * SHA2::SHA2256_RATE_SIZE, m_dgtState[i]);
				});
			...
			if (Length >= m_parallelProfile.ParallelBlockSize())
			{
				...
				{
					ProcessLeaf(Input, InOffset + (i * SHA2::SHA2256_RATE_SIZE), m_dgtState[i], PRCLEN);
				});
			...
			if (Length >= m_parallelProfile.ParallelMinimumSize())
			{
				...
				{
					ProcessLeaf(Input, InOffset + (i * SHA2::SHA2256_RATE_SIZE), m_dgtState[i], PRMLEN);
				});
		...
		{
			if (m_msgLength != 0 && (m_msgLength + Length >= SHA2::SHA2256_RATE_SIZE))
			{
				const size_t RMDLEN = SHA2::SHA2256_RATE_SIZE - m_msgLength;
				if (RMDLEN != 0)
				{
					MemoryTools::Copy(Input, InOffset, m_msgBuffer, m_msgLength, RMDLEN);
				}
			...
			while (Length >= SHA2::SHA2256_RATE_SIZE)
			{
				Permute(Input, InOffset, m_dgtState[0]);
				InOffset += SHA2::SHA2256_RATE_SIZE;
				Length -= SHA2::SHA2256_RATE_SIZE;
			}
		}
...
void SHA2256::HashFinal(std::vector<byte> &Input, size_t InOffset, size_t Length, SHA2256State &State)
{
	...
	if (Length < SHA2::SHA2256_RATE_SIZE)
	{
		MemoryTools::Clear(Input, InOffset + Length, SHA2::SHA2256_RATE_SIZE - Length);
	}

	if (Length > 56)
	{
		Permute(Input, InOffset, State);
		MemoryTools::Clear(Input, 0, SHA2::SHA2256_RATE_SIZE);
	}
