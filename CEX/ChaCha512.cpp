...
ChaCha512::ChaCha512(StreamAuthenticators AuthenticatorType)
	...
	m_authenticatorType(AuthenticatorType != StreamAuthenticators::KMAC1024 ? AuthenticatorType : 
		throw CryptoSymmetricException(CLASS_NAME, std::string("Constructor"), std::string("The authenticator must be a 256 or 512-bit MAC function!"), ErrorCodes::IllegalOperation)),
...
void ChaCha512::Initialize(bool Encryption, ISymmetricKey &Parameters)
{
	...
	{
		throw CryptoSymmetricException(Name(), std::string("Initialize"), std::string("The nonce is not required with ChaCha512!"), ErrorCodes::InvalidNonce);
	}
