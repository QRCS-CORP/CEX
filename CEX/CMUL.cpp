...
{
	const ulong X0 = IntegerTools::BeBytesTo64(Output, 0);
	const ulong X1 = IntegerTools::BeBytesTo64(Output, sizeof(ulong));
	...
	// round 1
	mask = IntegerTools::ExpandMask<ulong>(X0 & mpos);
	mpos >>= 1;
	Z0 ^= T0 & mask;
	Z1 ^= T1 & mask;
	...
	// round 64 + 1
	mask = IntegerTools::ExpandMask<ulong>(X1 & mpos);
	mpos >>= 1;
	Z0 ^= T0 & mask;
	Z1 ^= T1 & mask;
	carry = R & IntegerTools::ExpandMask<ulong>(T1 & 1);
	...
	IntegerTools::Be64ToBytes(Z1, Output, sizeof(ulong));
}

void CMUL::PermuteR128P128C(std::array<ulong, CMUL_STATE_SIZE> &State, std::array<byte, CMUL_BLOCK_SIZE> &Output)
{
	const ulong X0 = IntegerTools::BeBytesTo64(Output, 0);
	const ulong X1 = IntegerTools::BeBytesTo64(Output, sizeof(ulong));
	const ulong R = 0xE100000000000000ULL;
