...
TSX512::TSX512(StreamAuthenticators AuthenticatorType)
	:
	m_tsx512State(new TSX512State),
...
{
	return m_tsx512State->Encryption;
}

const bool TSX512::IsInitialized()
{
	return m_tsx512State->Initialized;
}
...
void TSX512::Initialize(bool Encryption, ISymmetricKey &Parameters)
{
	...
	// copy nonce
	m_tsx512State->Nonce[0] = IntegerTools::LeBytesTo64(Parameters.Nonce(), 0);
	m_tsx512State->Nonce[1] = IntegerTools::LeBytesTo64(Parameters.Nonce(), 8);
	...
	else
	{
		...
		MemoryTools::Copy(Name(), 0, m_tsx512State->Custom, sizeof(ulong), Name().size());
	...
	m_tsx512State->Encryption = Encryption;
	m_tsx512State->Initialized = true;
}
...
void TSX512::SetAssociatedData(const std::vector<byte> &Input, const size_t Offset, const size_t Length)
{
	if (!IsInitialized())
	{
...
void TSX512::Transform(const std::vector<byte> &Input, const size_t InOffset, std::vector<byte> &Output, const size_t OutOffset, const size_t Length)
{
	if (IsEncryption())
	{
		if (IsAuthenticator())
...
void TSX512::Generate(std::unique_ptr<TSX512State> &State, std::array<ulong, 2> &Counter, std::vector<byte> &Output, const size_t OutOffset, const size_t Length)
{
	size_t ctr;
...
void TSX512::Process(const std::vector<byte> &Input, const size_t InOffset, std::vector<byte> &Output, const size_t OutOffset, const size_t Length)
{
	...
	else
	{
		...
		ParallelTools::ParallelFor(0, m_parallelProfile.ParallelMaxDegree(), [this, &Input, InOffset, &Output, OutOffset, &tmpCtr, CNKLEN, CTROFT](size_t i)
		{
			...
			this->Generate(m_tsx512State, thdCtr, Output, OutOffset + (i * CNKLEN), CNKLEN);
