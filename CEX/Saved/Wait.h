...
protected:
	...
	virtual void Trace(uint N, const std::string &S) = 0;
	...
	void TraceIf(uint N, const std::string &S)
	{
		if (N)
		{
			Trace(N, S);
...
protected:
	...
	const char* m_info;
	const CallStack* m_prev;

public:

	CallStack(const char* I, const CallStack* P)
		: 
		m_info(I), 
		m_prev(P) 
	{
...
public:
	...
	CallStackWithNr(const char* I, uint N, const CallStack* P)
		: 
...
public:
	...
	void Clear();
	void SetNoWait(const CallStack &Stack);
	void ScheduleEvent(double MilliSeconds, const CallStack &Stack);
	// returns false if timed out
	bool Wait(ulong MilliSeconds);

#ifdef CEX_WINDOWS_SOCKETS
	~WaitObjectContainer();
	void AddHandle(HANDLE Handle, const CallStack &Stack);
#else
	void AddReadFd(int Fd, const CallStack &Stack);
	void AddWriteFd(int Fd, const CallStack &Stack);
#endif
...
private:
	...
	void DetectNoWait(LastResultType Result, const CallStack &Stack);
};
