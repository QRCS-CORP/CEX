...
#endif
	SecureVector<byte> Custom = { 0 };
	SecureVector<byte> MacKey = { 0 };
	SecureVector<byte> MacTag = { 0 };
	SecureVector<byte> Name = { 0 };
...
void ACS::Initialize(bool Encryption, ISymmetricKey &Parameters)
{
	...
	{
		m_acsState->Authenticator = (Parameters.KeySizes().KeySize() == IK128_SIZE) ?
			KmacModes::KMAC128 :
			(Parameters.KeySizes().KeySize() == IK512_SIZE) ?
			KmacModes::KMAC512 :
			KmacModes::KMAC256;
