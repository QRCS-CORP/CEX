...
gf2m polyn_gf2m::generate_gf2m_mask(gf2m a)
{
	gf2m result = (a != 0);
	return ~(result - 1);
}
...
*/
unsigned polyn_gf2m::nlz_16bit(uint16_t x)
{
	unsigned n;
	if (x == 0) return 16;
	n = 0;
...
int polyn_gf2m::calc_degree_secure() const
{
	...
	for (; i >= 0; i--)
	{
		found_mask = gf2m_small_m::expand_mask_16bit(this->coeff[i]);
...
gf2m polyn_gf2m::random_gf2m(Prng::IPrng* rng)
{
	return (gf2m)rng->NextUShort();
}

gf2m polyn_gf2m::random_code_element(unsigned code_length, Prng::IPrng* rng)
{
	if (code_length == 0)
	{
...
polyn_gf2m::polyn_gf2m(int d, GF2m_Field* sp_field)
	: 
	m_deg(-1),
	coeff(d + 1),
	msp_field(sp_field)
...
{
	this->coeff = std::vector<gf2m>(new_size);
}

polyn_gf2m::polyn_gf2m(const uint8_t* mem, uint32_t mem_len, GF2m_Field* sp_field)
	:
	msp_field(sp_field)
{
	...
	this->coeff = std::vector<gf2m>(size);
	this->m_deg = -1;
...
polyn_gf2m::polyn_gf2m(GF2m_Field* sp_field)
	: 
	m_deg(-1),
	coeff(1),
	msp_field(sp_field)
...
polyn_gf2m::polyn_gf2m(int degree, const unsigned  char* mem, uint32_t mem_byte_len, GF2m_Field* sp_field)
	:
	msp_field(sp_field)
{
	uint32_t j, k, l;
	...
	this->coeff = std::vector<gf2m>(degree + 1);
...
{
	//clear_mem(&this->coeff[0], this->coeff.size()); //TODO
	this->m_deg = -1;
}
...
gf2m polyn_gf2m::eval_aux(const gf2m * /*restrict*/ coeff, gf2m a, int d, GF2m_Field* sp_field)
{
	gf2m b;
	b = coeff[d--];
	for (; d >= 0; --d)
...
{
	int i, j, d;
	GF2m_Field* msp_field = g.msp_field;
...
std::vector<polyn_gf2m> polyn_gf2m::sqmod_init(const polyn_gf2m & g)
{
	...
	for (; i < d; ++i)
	{
		...
		std::memset(&sq[i].coeff[0], 0, 2);
		std::memcpy(&sq[i].coeff[0] + 2, &sq[i - 1].coeff[0], d);
...
polyn_gf2m polyn_gf2m::sqmod(const std::vector<polyn_gf2m> & sq, int d)
{
	int i, j;
	gf2m la;
	GF2m_Field* sp_field = this->msp_field;
...
polyn_gf2m::polyn_gf2m(int t, Prng::IPrng* rng, GF2m_Field* sp_field)
	:
	m_deg(t),
	coeff(t + 1),
	msp_field(sp_field)
...
std::vector<polyn_gf2m> polyn_gf2m::syndrome_init(polyn_gf2m const& generator, std::vector<gf2m> const& support, int n)
{
	int i, j, t;
	gf2m a;

	GF2m_Field* msp_field = generator.msp_field;
	...
	for (j = 0; j<n; j++)
	{
		...
		for (i = t - 2; i >= 0; i--)
		{
			(*&result[j]).set_coef(i, (generator)[i + 1] ^
				msp_field->gf_mul(gf2m_small_m::lex_to_gray(support[j]), result[j][i + 1]));
		}
		a = ((generator)[0] ^ msp_field->gf_mul(gf2m_small_m::lex_to_gray(support[j]), result[j][0]));
		for (i = 0; i<t; i++)
		{
...
polyn_gf2m::polyn_gf2m(const std::vector<uint8_t>& encoded, GF2m_Field* sp_field)
	:
	msp_field(sp_field)
{
	if (encoded.size() % 2)
...
std::vector<uint8_t> polyn_gf2m::encode() const
{
