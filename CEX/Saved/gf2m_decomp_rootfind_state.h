#ifndef _CEX_GF2MROOTFINDSTATE_H
#define _CEX_GF2MROOTFINDSTATE_H
...
private:
	...
	std::vector<gf2m> m_Lik; // size is outer_summands * m
	std::vector<gf2m> m_Aij; // ...
...
public:
	...
	gf2m_decomp_rootfind_state(const polyn_gf2m &polyn, uint32_t the_code_length)
		:
		code_length(the_code_length), m_j(0), m_j_gray(0)
	{
		gf2m coeff_3;
		gf2m coeff_head;
		GF2m_Field* sp_field = polyn.get_sp_field();
	...
	*/
	...
	{
		GF2m_Field* sp_field = sigma.get_sp_field();
		uint32_t i, k, d;
		d = sigma.get_degree();
	...
	void calc_next_Aij()
	{
		...
		*/
		...
		new_j_gray = gf2m_small_m::lex_to_gray(this->m_j);
	...
	std::vector<gf2m> gf2m_decomp_rootfind_state::find_roots(const polyn_gf2m & sigma)
	{
		const int sigma_degree = sigma.get_degree();
		//BOTAN_ASSERT(sigma_degree > 0, "Valid sigma");
		std::vector<gf2m> result(sigma_degree);
		uint32_t root_pos = 0;
	...
	uint32_t patch_root_array(gf2m* res_root_arr, uint32_t res_root_arr_len, uint32_t root_pos)
	{
		...
		cond_mask = gf2m_small_m::expand_mask_16bit(cond_mask);
