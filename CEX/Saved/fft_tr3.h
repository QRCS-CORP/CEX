#ifndef _CEX_FFTTR3_H
#define _CEX_FFTTR3_H
...
#define vec_add(z, x, y)           \
	...
	static void radix_conversions_tr(std::vector<std::vector<ulong>> &in, size_t Dimension)
	{
		int i, j, k;
		...
		const std::vector<std::vector<std::vector<ulong>>> s =
		{
			{ 
				{
					0XF3CFC030FC30F003,
		...
		for (j = 5; j >= 0; j--)
		{
			if (j < 5)
			{
				GfVector::vec_mul(in[0], 0, in[0], 0, s[j][0], 0, Dimension); // scaling
				GfVector::vec_mul(in[1], 0, in[1], 0, s[j][1], 0, Dimension); // scaling
			}
	...
	static void butterflies_tr(std::vector<std::vector<ulong>> &out, std::vector<std::vector<ulong>> &in, size_t Dimension)
	{
		int i, j, k, s, b;

		std::vector<ulong> tmp(Dimension);
		std::vector<std::vector<ulong>> pre(6, std::vector<ulong>(Dimension));
		std::vector<ulong> buf(64);

		const std::vector<std::vector<ulong>> consts =
		{
			//64
			{
				0XF00F0FF0F00F0FF0,
		...
		const std::vector<ushort> beta = { 8, 1300, 3408, 1354, 2341, 1154 };
		...
		for (i = 5; i >= 0; i--)
		{
			...
			for (j = 0; j < 64; j += 2 * s)
			{
				for (k = j; k < j + s; k++)
				{
					...
					GfVector::vec_mul(tmp, 0, in[k], 0, consts[consts_ptr + (k - j)], 0, Dimension);
		...
		for (i = 0; i < Dimension; i++)
		{
			...
			Transpose2::transpose_64x64_compact(buf, buf);
		...
		GfVector::vec_copy(pre[0], in[32], Dimension);
		vec_add(in[33], in[33], in[32]);
		GfVector::vec_copy(pre[1], in[33], Dimension);
		vec_add(in[35], in[35], in[33]);
		vec_add(pre[0], pre[0], in[35]);
		vec_add(in[34], in[34], in[35]);
		GfVector::vec_copy(pre[2], in[34], Dimension);
		...
		GfVector::vec_copy(pre[3], in[36], Dimension);
		...
		GfVector::vec_copy(pre[4], in[40], Dimension);
		...
		GfVector::vec_copy(pre[5], in[48], Dimension);
		...
		GfVector::vec_mul(out[1], 0, pre[0], 0, tmp, 0, Dimension);
		...
		for (i = 1; i < 6; i++)
		{
			...
			GfVector::vec_mul(tmp, 0, pre[i], 0, tmp, 0, Dimension);
			vec_add(out[1], out[1], tmp);
		}
	}

	static void fft_tr(std::vector<std::vector<ulong>> &out, std::vector<std::vector<ulong>> &in, size_t Dimension)
	{
		butterflies_tr(out, in, Dimension);
		radix_conversions_tr(out, Dimension);
	}
