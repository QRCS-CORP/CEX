...
public:
	...
	/*static void vec_add(std::vector<ulong> &z, std::vector<ulong> &x, const std::vector<ulong> &y, int GfBits)
	{
		for (size_t i = 0; i < GfBits; i++)
			z[i] = x[i] ^ y[i];
	}*/

	static void vec_mul(std::vector<ulong> &h, size_t hOffset, std::vector<ulong> &f, size_t fOffset, const std::vector<ulong> &g, size_t gOffset, int GfBits)
	{
		std::vector<ulong> result(2 * GfBits - 1);

		uint64_t t1 = f[fOffset + 11] & g[gOffset + 11];
		uint64_t t2 = f[fOffset + 11] & g[gOffset + 9];
		uint64_t t3 = f[fOffset + 11] & g[gOffset + 10];
		uint64_t t4 = f[fOffset + 9] & g[gOffset + 11];
		uint64_t t5 = f[fOffset + 10] & g[gOffset + 11];
		uint64_t t6 = f[fOffset + 10] & g[gOffset + 10];
		uint64_t t7 = f[fOffset + 10] & g[gOffset + 9];
		uint64_t t8 = f[fOffset + 9] & g[gOffset + 10];
		uint64_t t9 = f[fOffset + 9] & g[gOffset + 9];
		uint64_t t10 = t8 ^ t7;
		uint64_t t11 = t6 ^ t4;
		uint64_t t12 = t11 ^ t2;
		uint64_t t13 = t5 ^ t3;
		uint64_t t14 = f[fOffset + 8] & g[gOffset + 8];
		uint64_t t15 = f[fOffset + 8] & g[gOffset + 6];
		uint64_t t16 = f[fOffset + 8] & g[gOffset + 7];
		uint64_t t17 = f[fOffset + 6] & g[gOffset + 8];
		uint64_t t18 = f[fOffset + 7] & g[gOffset + 8];
		uint64_t t19 = f[fOffset + 7] & g[gOffset + 7];
		uint64_t t20 = f[fOffset + 7] & g[gOffset + 6];
		uint64_t t21 = f[fOffset + 6] & g[gOffset + 7];
		uint64_t t22 = f[fOffset + 6] & g[gOffset + 6];
		uint64_t t23 = t21 ^ t20;
		uint64_t t24 = t19 ^ t17;
		uint64_t t25 = t24 ^ t15;
		uint64_t t26 = t18 ^ t16;
		uint64_t t27 = f[fOffset + 5] & g[gOffset + 5];
		uint64_t t28 = f[fOffset + 5] & g[gOffset + 3];
		uint64_t t29 = f[fOffset + 5] & g[gOffset + 4];
		uint64_t t30 = f[fOffset + 3] & g[gOffset + 5];
		uint64_t t31 = f[fOffset + 4] & g[gOffset + 5];
		uint64_t t32 = f[fOffset + 4] & g[gOffset + 4];
		uint64_t t33 = f[fOffset + 4] & g[gOffset + 3];
		uint64_t t34 = f[fOffset + 3] & g[gOffset + 4];
		uint64_t t35 = f[fOffset + 3] & g[gOffset + 3];
		uint64_t t36 = t34 ^ t33;
		uint64_t t37 = t32 ^ t30;
		uint64_t t38 = t37 ^ t28;
		uint64_t t39 = t31 ^ t29;
		uint64_t t40 = f[fOffset + 2] & g[gOffset + 2];
		uint64_t t41 = f[fOffset + 2] & g[gOffset + 0];
		uint64_t t42 = f[fOffset + 2] & g[gOffset + 1];
		uint64_t t43 = f[fOffset + 0] & g[gOffset + 2];
		uint64_t t44 = f[fOffset + 1] & g[gOffset + 2];
		uint64_t t45 = f[fOffset + 1] & g[gOffset + 1];
		uint64_t t46 = f[fOffset + 1] & g[gOffset + 0];
		uint64_t t47 = f[fOffset + 0] & g[gOffset + 1];
		uint64_t t48 = f[fOffset + 0] & g[gOffset + 0];
		...
		uint64_t t59 = g[gOffset + 6] ^ g[gOffset + 9];
		uint64_t t60 = g[gOffset + 7] ^ g[gOffset + 10];
		uint64_t t61 = g[gOffset + 8] ^ g[gOffset + 11];
		uint64_t t62 = f[fOffset + 6] ^ f[fOffset + 9];
		uint64_t t63 = f[fOffset + 7] ^ f[fOffset + 10];
		uint64_t t64 = f[fOffset + 8] ^ f[fOffset + 11];
		...
		uint64_t t78 = g[gOffset + 0] ^ g[gOffset + 3];
		uint64_t t79 = g[gOffset + 1] ^ g[gOffset + 4];
		uint64_t t80 = g[gOffset + 2] ^ g[gOffset + 5];
		uint64_t t81 = f[fOffset + 0] ^ f[fOffset + 3];
		uint64_t t82 = f[fOffset + 1] ^ f[fOffset + 4];
		uint64_t t83 = f[fOffset + 2] ^ f[fOffset + 5];
		...
		uint64_t t118 = g[gOffset + 3] ^ g[gOffset + 9];
		uint64_t t119 = g[gOffset + 4] ^ g[gOffset + 10];
		uint64_t t120 = g[gOffset + 5] ^ g[gOffset + 11];
		uint64_t t121 = g[gOffset + 0] ^ g[gOffset + 6];
		uint64_t t122 = g[gOffset + 1] ^ g[gOffset + 7];
		uint64_t t123 = g[gOffset + 2] ^ g[gOffset + 8];
		uint64_t t124 = f[fOffset + 3] ^ f[fOffset + 9];
		uint64_t t125 = f[fOffset + 4] ^ f[fOffset + 10];
		uint64_t t126 = f[fOffset + 5] ^ f[fOffset + 11];
		uint64_t t127 = f[fOffset + 0] ^ f[fOffset + 6];
		uint64_t t128 = f[fOffset + 1] ^ f[fOffset + 7];
		uint64_t t129 = f[fOffset + 2] ^ f[fOffset + 8];
		...
		for (size_t i = 2 * GfBits - 2; i >= GfBits; i--) 
		{
			result[i - 9] ^= result[i];
		...
		for (size_t i = 0; i < GfBits; i++)
			h[hOffset + i] = result[i];
	}

	static void vec_sq(std::vector<ulong> &out, std::vector<ulong> &in, int GfBits)
	{
		int i;
		std::vector<ulong> result(GfBits);
	...
	static void vec_copy(std::vector<ulong> &out, const std::vector<ulong> &in, int GfBits)
	{
		for (size_t i = 0; i < GfBits; i++)
			out[i] = in[i];
	}

	static uint64_t vec_or(const std::vector<ulong> &in, int GfBits)
	{
		uint64_t ret = in[0];
	...
	static void vec_inv(std::vector<ulong> &out, const std::vector<ulong> &in, int GfBits)
	{
		std::vector<ulong> tmp_11(GfBits);
		std::vector<ulong> tmp_1111(GfBits);

		vec_copy(out, in, GfBits);

		vec_sq(out, out, GfBits);
		vec_mul(tmp_11, 0, out, 0, in, 0, GfBits);

		vec_sq(out, tmp_11, GfBits);
		vec_sq(out, out, GfBits);
		vec_mul(tmp_1111, 0, out, 0, tmp_11, 0, GfBits);

		vec_sq(out, tmp_1111, GfBits);
		vec_sq(out, out, GfBits);
		vec_sq(out, out, GfBits);
		vec_sq(out, out, GfBits);
		vec_mul(out, 0, out, 0, tmp_1111, 0, GfBits);

		vec_sq(out, out, GfBits);
		vec_sq(out, out, GfBits);
		vec_mul(out, 0, out, 0, tmp_11, 0, GfBits);

		vec_sq(out, out, GfBits);
		vec_mul(out, 0, out, 0, in, 0, GfBits);
