...
void LimitedBandwidth::ComputeNextTransceiveTime()
{
	...
	m_nextTransceiveTime = (total < m_maxBytesPerSecond) ? curTime : m_ops.front().first + 1000;
}
...
/*size_t NonblockingSource::GeneralPump2(lword& byteCount, bool blockingOutput, unsigned long maxTime, bool checkDelimiter, byte delimiter)
{
...
size_t NetworkSource::DoPump(lword &byteCount, bool blockingOutput, unsigned long maxTime, bool checkDelimiter, byte delimiter)
{
	...
	while (true)
	{
		if (m_dataBegin == m_dataEnd)
		{
			...
			{
				if (receiver.MustWaitForResult() && !receiver.Wait(SaturatingSubtract(maxTime, timer.ElapsedTime()), CallStack("NetworkSource::DoPump() - wait receive result", 0)))
				{
					break;
				}
			...
			else
			{
				...
				{
					if (!receiver.Wait(SaturatingSubtract(maxTime, timer.ElapsedTime()), CallStack("NetworkSource::DoPump() - wait receive", 0)))
					{
						break;
					}
		...
		DoOutput:
			...
			if (result)
			{
				if (t->Wait(SaturatingSubtract(maxTime, timer.ElapsedTime()), CallStack("NetworkSource::DoPump() - wait attachment", 0)))
				{
					goto DoOutput;
				}
				else
...
/*NetworkSink::NetworkSink(unsigned int maxBufferSize, unsigned int autoFlushBound)
	...
	m_autoFlushBound(autoFlushBound), 
	m_needSendResult(false), 
	m_wasBlocked(false), 
	m_eofState(EOF_NONE),
	m_buffer(STDMIN(16U * 1024U + 256, maxBufferSize)),
	m_skipBytes(0),
	m_speedTimer(Timer::MILLISECONDS),
	m_byteCountSinceLastTimerReset(0), 
	m_currentSpeed(0), 
	m_maxObservedSpeed(0)
{
}
...
lword NetworkSink::DoFlush(unsigned long maxTime, size_t targetSize)
{
	...
	while (true)
	{
		...
		if (m_needSendResult)
		{
			if (sender.MustWaitForResult() && !sender.Wait(SaturatingSubtract(maxTime, timer.ElapsedTime()), CallStack("NetworkSink::DoFlush() - wait send result", 0)))
			{
				break;
			}
