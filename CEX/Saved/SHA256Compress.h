...
public:
	...
	template <typename T>
	static void Compress64W(const std::vector<byte> &Input, size_t InOffset, T &Output)
	{
#if defined(__AVX__)
		__m128i S0, S1, T0, T1;
	...
	template <typename T>
	static void Compress64(const std::vector<byte> &Input, size_t InOffset, T &Output)
	{
		uint A = Output.H[0];
		uint B = Output.H[1];
		uint C = Output.H[2];
