...
void SPXPS128SHAKE::Generate(std::vector<uint8_t> &PublicKey, std::vector<uint8_t> &PrivateKey, std::unique_ptr<Prng::IPrng> &Rng)
{
	std::vector<uint32_t> heights(SPX_TREE_HEIGHT + 1);
	std::vector<uint8_t> stack((SPX_TREE_HEIGHT + 1) * SPX_N);
	std::vector<uint8_t> authpath(SPX_TREE_HEIGHT * SPX_N);
	std::vector<uint8_t> root(SPX_N);
	std::vector<uint8_t> pkseed(SPX_N);
	std::vector<uint8_t> skseed(SPX_N);
	std::vector<uint8_t> stmp(3 * SPX_N);
...
size_t SPXPS128SHAKE::Sign(std::vector<uint8_t> &Signature, const std::vector<uint8_t> &Message, const std::vector<uint8_t> &PrivateKey, std::unique_ptr<Prng::IPrng> &Rng)
{
	// returns an array containing the signature followed by the message

	std::vector<uint32_t> heights(SPX_FORS_HEIGHT + 1);
	std::vector<uint8_t> optrand(SPX_N);
	std::vector<uint8_t> pk(2 * SPX_N);
	std::vector<uint8_t> root(SPX_N);
	std::vector<uint8_t> skseed(SPX_N);
	std::vector<uint8_t> stack((SPX_FORS_HEIGHT + 1) * SPX_N);
	std::vector<uint8_t> mhash(SPX_FORS_MSG_BYTES);
	std::vector<uint8_t> skprf(SPX_N);
...
bool SPXPS128SHAKE::Verify(std::vector<uint8_t> &Message, const std::vector<uint8_t> &Signature, const std::vector<uint8_t> &PublicKey)
{
	// verifies a given signature-message pair under a given public key
	const size_t MSGLEN = Signature.size() - SPX_BYTES;
	std::vector<uint8_t> buf(SPX_N + SPX_ADDR_BYTES + SPX_WOTS_LEN * SPX_N);
	std::vector<uint8_t> leaf(SPX_N);
	std::vector<uint8_t> mask(SPX_WOTS_LEN * SPX_N);
	std::vector<uint8_t> pkroot(SPX_N);
	std::vector<uint8_t> pkseed(SPX_N);
	std::vector<uint8_t> root(SPX_N);
	std::vector<uint8_t> tmsg(Signature.size());
	std::vector<uint8_t> wotspk(SPX_WOTS_BYTES);
	uint64_t tree;
	size_t idxsig;
	uint32_t idx;
	uint32_t idxleaf;
	...
	{
		std::vector<uint8_t> sig(SPX_BYTES);
		std::vector<uint8_t> mhash(SPX_FORS_MSG_BYTES);
		std::array<uint32_t, 8> treeaddr = { 0 };
		std::array<uint32_t, 8> wotsaddr = { 0 };
