...
RingLWE::RingLWE(RLWEParams Parameters, Prngs PrngType, Digests DigestType, bool Parallel)
	...
	m_dgtFinalizer(Helper::DigestFromName::GetInstance(DigestType)),
	m_isDestroyed(false),
...
std::vector<byte> RingLWE::Decrypt(std::vector<byte> &CipherText)
{
	...
	if (m_rlweParameters == RLWEParams::Q12289N1024)
	{
		...
		FFTQ12289N1024::SharedA(secret, m_privateKey->R(), CipherText, m_dgtFinalizer);
...
void RingLWE::Destroy()
{
	if (!m_isDestroyed)
	{
		...
		if (m_destroyEngine)
		{
			...
			if (m_dgtFinalizer != 0)
			{
				delete m_dgtFinalizer;
			}
		}
	}
}

std::vector<byte> RingLWE::Encrypt(std::vector<byte> &Message)
{
	CEXASSERT(m_isInitialized, "The cipher has not been initialized");
	...
	if (m_rlweParameters == RLWEParams::Q12289N1024)
	{
		...
		if (Message.size() != FFTQ12289N1024::SEED_BYTES)
		{
			Message.resize(FFTQ12289N1024::SEED_BYTES);
		}

		std::vector<byte> reply(FFTQ12289N1024::SENDB_BYTES);
		// generate B reply and copy shared secret to input
		FFTQ12289N1024::SharedB(Message, reply, m_publicKey->P(), m_rndGenerator, m_dgtFinalizer, m_isParallel);
