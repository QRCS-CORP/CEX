...
const std::string FFTQ12289N512::Name = "Q12289N1024";

const std::array<ushort, 512> FFTQ12289N512::BitrevTable =
{
...
const std::array<ushort, 256> FFTQ12289N512::OmegasMontgomery =
{
...
const std::array<ushort, 256> FFTQ12289N512::OmegasInvMontgomery =
{
...
const std::array<ushort, 512> FFTQ12289N512::PsisBitrevMontgomery =
{
	0x0FEBU, 0x14C3U, 0x1F1DU, 0x1CCDU, 0x020AU, 0x2788U, 0x2343U, 0x13D7U, 0x0928U, 0x04FEU, 0x07B5U, 0x15C6U, 0x03FAU, 0x18DCU, 0x2BF0U, 0x2247U,
	0x1D4CU, 0x1E8EU, 0x15A1U, 0x128DU, 0x2134U, 0x2F6EU, 0x1550U, 0x1EA0U, 0x1545U, 0x0F14U, 0x11B8U, 0x2BE7U, 0x181BU, 0x2117U, 0x0A7BU, 0x2B5BU,
	0x2941U, 0x0190U, 0x17F9U, 0x1CADU, 0x1527U, 0x21C6U, 0x17F8U, 0x16E6U, 0x1599U, 0x1456U, 0x0038U, 0x2382U, 0x2214U, 0x2D73U, 0x06A6U, 0x283EU,
...
const std::array<ushort, 512> FFTQ12289N512::PsisInvMontgomery =
{
...
void FFTQ12289N512::Decrypt(std::vector<byte> &Secret, const std::vector<ushort> &PrivateKey, const std::vector<byte> &Received)
{
...
void FFTQ12289N512::Encrypt(std::vector<byte> &Secret, std::vector<byte> &Send, const std::vector<byte> &Received, std::unique_ptr<Prng::IPrng> &Rng, bool Parallel)
{
...
void FFTQ12289N512::Generate(std::vector<byte> &PublicKey, std::vector<ushort> &PrivateKey, std::unique_ptr<Prng::IPrng> &Rng, bool Parallel)
{
	std::array<ushort, N> a;
...
void FFTQ12289N512::DecodeA(std::array<ushort, N> &PubKey, std::vector<byte> &Seed, const std::vector<byte> &R)
{
	FromBytes(PubKey, R);
	Utility::MemUtils::Copy(R, POLY_BYTES, Seed, 0, SEED_BYTES);
}

void FFTQ12289N512::DecodeB(std::array<ushort, N> &B, std::array<ushort, N> &C, const std::vector<byte> &R)
{
	FromBytes(B, R);
...
void FFTQ12289N512::EncodeA(std::vector<byte> &R, const std::array<ushort, N> &PubKey, const std::vector<byte> &Seed)
{
	ToBytes(R, PubKey);
	Utility::MemUtils::Copy(Seed, 0, R, POLY_BYTES, SEED_BYTES);
}

void FFTQ12289N512::EncodeB(std::vector<byte> &R, const std::array<ushort, N> &B, const std::array<ushort, N> &C)
{
	ToBytes(R, B);
...
void FFTQ12289N512::FromBytes(std::array<ushort, N> &R, const std::vector<byte> &A)
{
	for (size_t i = 0; i < N / 4; ++i)
	{
...
void FFTQ12289N512::PolyUniform(std::array<ushort, N> &A, const std::vector<byte> &Seed, bool Parallel)
{
...
void FFTQ12289N512::RecHelper(std::array<ushort, N> &C, const std::array<ushort, N> &V, std::vector<byte> &Random)
{
#if defined(__AVX512__)
...
void FFTQ12289N512::Reconcile(std::vector<byte> &Key, const std::array<ushort, N> &V, const std::array<ushort, N> &C)
{
#if defined(__AVX512__)
...
void FFTQ12289N512::ToBytes(std::vector<byte> &R, const std::array<ushort, N> &Poly)
{
	short c;
	ushort m;
	ushort t0;
