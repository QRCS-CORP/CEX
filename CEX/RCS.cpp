...
const std::vector<byte> RCS::RCS_INFO = 
{
...
void RCS::Initialize(bool Encryption, ISymmetricKey &Parameters)
{
	...
	m_rcsState->Custom.resize(Parameters.KeySizes().InfoSize() + RCS_INFO.size());
	// copy the version string to the customization parameter
	MemoryTools::Copy(RCS_INFO, 0, m_rcsState->Custom, 0, RCS_INFO.size());
	// copy the user defined string to the customization parameter
	MemoryTools::Copy(Parameters.Info(), 0, m_rcsState->Custom, RCS_INFO.size(), Parameters.KeySizes().InfoSize());
	...
	const size_t RNKLEN = (BLOCK_SIZE / sizeof(uint)) * (m_rcsState->Rounds + 1U);
...
void RCS::Transform256(const std::vector<byte> &Input, size_t InOffset, std::vector<byte> &Output, size_t OutOffset)
{
	...
	for (i = 1; i < m_rcsState->Rounds; ++i)
	{
		...
		ShiftRows256(state);
		MixColumns(state);
		KeyAddition(state, m_rcsState->RoundKeys, (i << 3));
	}

	Substitution(state);
	ShiftRows256(state);
	KeyAddition(state, m_rcsState->RoundKeys, (m_rcsState->Rounds << 3));
