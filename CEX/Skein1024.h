...
/// <summary>
/// An implementation of the Skein sequential and parallel message-digests with a 1024-bit hash code
/// </summary> 
/// 
/// <example>
...
/// The default settings are applied when using the Skein1024(bool) constructor, or a manually defined configuration when using the Skein1024(SkeinParams) constructor. \n
/// The SkeinParams structure contains property accessors that are initialized by the boolean constructor at their defaults; 
/// Schema(83,72,65,51) OuputSize(128), Version(1), FanOut(Parallel ? 8 : 0), and LeafSize(128). \n
...
/// The DistributionCodeMax() accessor provides the upper limit to the personalization strings uint8_t-length, this size is different for each variant; (256=8, 512=40, 1024=104).</para>
///
...
/// </list>
/// </remarks>
class Skein1024 final : public IDigest
{
private:

	static const uint8_t DEF_PRLDEGREE = 8;
	static const uint8_t MAX_PRLDEGREE = 64;
	// size of reserved state buffer subtracted from parallel size calculations
	static const size_t STATE_PRECACHED = 2048;

	class Skein1024State;
...
private:
	...
	static void HashFinal(std::vector<uint8_t> &Input, size_t InOffset, size_t Length, Skein1024State &State);
	static void Initialize(std::vector<Skein1024State> &State, SkeinParams &Params);
	static void LoadState(Skein1024State &State, std::array<uint64_t, 16> &Config);
	static void Permute(std::array<uint64_t, 16> &Message, Skein1024State &State);
	static void ProcessBlock(const std::vector<uint8_t> &Input, size_t InOffset, Skein1024State &State, size_t Length);
	void ProcessLeaf(const std::vector<uint8_t> &Input, size_t InOffset, Skein1024State &State, uint64_t Length);
};
