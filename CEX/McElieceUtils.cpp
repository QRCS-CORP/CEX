...
void McElieceUtils::LayerIn(ulong Data[2][64], const ulong* Bits, uint Lgs)
{
...
void McElieceUtils::LayerEx(ulong* Data, const ulong* Bits, uint Lgs)
{
...
void McElieceUtils::ApplyBenes(byte* R, const byte* Bits, bool Reverse)
{
...
void McElieceUtils::Compose(uint W, uint N, const uint* Pi, uint* P)
{
	uint* I = new uint[2 * N];
	uint* Ip = new uint[N];
	size_t i;
...
void McElieceUtils::ComposeInv(uint N, uint* Y, const uint* X, const uint* Pi)
{
	// y[pi[i]] = x[i]
	// requires n = 2^w
...
void McElieceUtils::CSwap(uint &X, uint &Y, byte Swap)
{
	uint m;
	uint d;
...
void McElieceUtils::CSwap63b(ulong &X, ulong &Y, byte Swap)
{
	ulong m;
	ulong d;
...
void McElieceUtils::Flow(uint W, uint &X, const uint &Y, const uint T)
{
	uint b;
	uint ycopy;
	byte m0;
...
void McElieceUtils::Invert(uint N, uint* Ip, const uint* Pi)
{
	// ip[i] = j iff pi[i] = j
	// requires n = 2^w
	// requires pi to be a permutation
...
byte McElieceUtils::IsSmaller(uint A, uint B)
{
	uint ret;
...
byte McElieceUtils::IsSmaller63b(ulong A, ulong B)
{
	ulong ret;
...
void McElieceUtils::Merge(uint N, uint* X, uint Step)
{
...
void McElieceUtils::Merge63b(uint N, ulong* X, uint Step)
{
	size_t i;
...
void McElieceUtils::MinMax(uint &X, uint &Y)
{
	byte m;
...
void McElieceUtils::MinMax63b(ulong &X, ulong &Y)
{
	byte m;
...
void McElieceUtils::Permute(uint W, uint N, uint Offset, uint Step, const uint* P, const uint* Pi, byte* C, uint* PiFlip)
{
	size_t i;
	size_t j;
...
void McElieceUtils::PermuteBits(uint W, uint N, uint Step, uint Offset, byte* C, const uint* Pi)
{
...
void McElieceUtils::Sort(uint N, uint* X)
{
...
void McElieceUtils::Sort63b(uint N, ulong* X)
{
	if (N > 1)
	{
		Sort63b(N / 2, X);
...
void McElieceUtils::Transpose64x64(ulong* Output, const ulong* Input)
{
...
ushort McElieceUtils::BitReverse(ushort A)
{
...
void McElieceUtils::Clear8(byte* A, size_t Count)
{
	size_t i;
...
void McElieceUtils::Clear32(uint* A, size_t Count)
{
	size_t i;
...
void McElieceUtils::Clear64(ulong* A, size_t Count)
{
	size_t i;
...
uint McElieceUtils::Le8To32(const byte* Input)
{
...
ulong McElieceUtils::Le8To64(const byte* Input)
{
	return ((ulong)Input[0]) |
		((ulong)Input[1] << 8) |
		((ulong)Input[2] << 16) |
...
void McElieceUtils::Le32To8(byte* Output, uint Value)
{
	Output[0] = Value & 0xFF;
...
void McElieceUtils::Le64To8(byte* Output, ulong Value)
{
	Output[0] = Value & 0xFF;
	Output[1] = (Value >> 8) & 0xFF;
	Output[2] = (Value >> 16) & 0xFF;
...
ushort McElieceUtils::Load16(const byte* Input)
{
	ushort a;
...
ulong McElieceUtils::Load64(const byte* Input)
{
	int32_t i;
	ulong ret;
...
uint McElieceUtils::Rotl32(uint Value, uint Shift)
{
	return (Value << Shift) | (Value >> ((sizeof(uint) * 8) - Shift));
}

ulong McElieceUtils::Rotl64(ulong Value, uint Shift)
{
	return (Value << Shift) | (Value >> ((sizeof(ulong) * 8) - Shift));
}

uint McElieceUtils::Rotr32(uint Value, uint Shift)
{
	return (Value >> Shift) | (Value << ((sizeof(uint) * 8) - Shift));
}

ulong McElieceUtils::Rotr64(ulong Value, uint Shift)
{
	return (Value >> Shift) | (Value << ((sizeof(ulong) * 8) - Shift));
}

void McElieceUtils::Store16(byte* Output, ushort A)
{
	Output[0] = A & 0xFF;
	Output[1] = A >> 8;
}

void McElieceUtils::Store64(byte* Output, ulong Input)
{
	Output[0] = Input & 0xFF;
...
int32_t McElieceUtils::Verify(const byte* A, const byte* B, size_t Length)
{
	size_t i;
	ushort d;
