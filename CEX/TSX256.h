...
/// An optional authentication component has also been added, and the cipher output can be authenticated using either an SHA2 hash based HMAC, the Keccak based KMAC, or Poly1305 message authentication code generators.</para>
///
...
/// The supported MAC generator options for TSX256 are Poly1305, HMAC(SHA2-256), and the Keccak based KMAC-256.</para>
///
...
private:
	...
	static void Generate(std::unique_ptr<TSX256State> &State, std::array<ulong, 2> &Counter, std::vector<byte> &Output, size_t OutOffset, size_t Length);
