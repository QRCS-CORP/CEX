...
/// <para>Used in conjunction with the <see cref="Crypto.Processing::MacStream"/> class. 
...
public:
		...
		m_keyId(0)
	{
		...
		{
			throw CryptoProcessingException("CipherKey:CTor", "The MacKey must be exactly 16 bytes!");
		}
		else
		{
			m_keyId = KeyId;
	...
	void Reset()
	{
		m_macDsc.Reset();
		if (m_keyId.size() != 0)
			ArrayUtils::ClearVector(m_keyId);
	}
