...
ChaCha20::ChaCha20(size_t Rounds)
	...
	m_rndCount(((Rounds <= MAX_ROUNDS) && (Rounds >= MIN_ROUNDS) && (Rounds % 2 == 0)) ? Rounds :
		throw CryptoSymmetricCipherException("ChaCha20:CTor", "Invalid rounds count! Sizes supported are even numbers between 8 and 80")),
	m_wrkState(14, 0)
{
	Scope();
}
...
const size_t ChaCha20::BlockSize() 
{ 
	return BLOCK_SIZE; 
}

const std::vector<byte> &ChaCha20::DistributionCode()
{ 
	return m_dstCode; 
}

const StreamCiphers ChaCha20::Enumeral() 
{
	return StreamCiphers::ChaCha20; 
}

const bool ChaCha20::IsInitialized() 
{ 
	return m_isInitialized;
}

const bool ChaCha20::IsParallel() 
{ 
	return m_parallelProfile.IsParallel();
}

const std::vector<SymmetricKeySize> &ChaCha20::LegalKeySizes() 
{
	return m_legalKeySizes; 
}

const std::vector<size_t> &ChaCha20::LegalRounds() 
{ 
...
void ChaCha20::TransformBlock(const std::vector<byte> &Input, std::vector<byte> &Output)
{
	Process(Input, 0, Output, 0, BLOCK_SIZE);
}

void ChaCha20::TransformBlock(const std::vector<byte> &Input, const size_t InOffset, std::vector<byte> &Output, const size_t OutOffset)
{
...
void ChaCha20::Expand(const std::vector<byte> &Key, const std::vector<byte> &Iv)
{
	if (Key.size() == 32)
	{
		...
		m_wrkState[1] = IntUtils::LeBytesTo32(m_dstCode, 4);
		m_wrkState[2] = IntUtils::LeBytesTo32(m_dstCode, 8);
		m_wrkState[3] = IntUtils::LeBytesTo32(m_dstCode, 12);
		m_wrkState[4] = IntUtils::LeBytesTo32(Key, 0);
		m_wrkState[5] = IntUtils::LeBytesTo32(Key, 4);
		m_wrkState[6] = IntUtils::LeBytesTo32(Key, 8);
		m_wrkState[7] = IntUtils::LeBytesTo32(Key, 12);
		m_wrkState[8] = IntUtils::LeBytesTo32(Key, 16);
		m_wrkState[9] = IntUtils::LeBytesTo32(Key, 20);
		m_wrkState[10] = IntUtils::LeBytesTo32(Key, 24);
		m_wrkState[11] = IntUtils::LeBytesTo32(Key, 28);
		m_wrkState[12] = IntUtils::LeBytesTo32(Iv, 0);
		m_wrkState[13] = IntUtils::LeBytesTo32(Iv, 4);
	...
	else
	{
		...
		m_wrkState[6] = IntUtils::LeBytesTo32(Key, 8);
		m_wrkState[7] = IntUtils::LeBytesTo32(Key, 12);
		m_wrkState[8] = IntUtils::LeBytesTo32(Key, 0);
		m_wrkState[9] = IntUtils::LeBytesTo32(Key, 4);
		m_wrkState[10] = IntUtils::LeBytesTo32(Key, 8);
		m_wrkState[11] = IntUtils::LeBytesTo32(Key, 12);
		m_wrkState[12] = IntUtils::LeBytesTo32(Iv, 0);
		m_wrkState[13] = IntUtils::LeBytesTo32(Iv, 4);
	}
}
