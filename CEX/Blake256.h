...
/// </list>
/// </remarks>
class Blake256 final : public IDigest
{
private:
...
private:
	...
	static void LoadState(BlakeParams &Params, std::vector<uint32_t> &Config, Blake2sState &State);
	static void Permute(const std::vector<uint8_t> &Input, size_t InOffset, Blake2sState &State);
	void ProcessLeaf(const std::vector<uint8_t> &Input, size_t InOffset, size_t Length, Blake2sState &State);
};
