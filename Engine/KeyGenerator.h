...
/// <seealso cref="CEX::Prng ">CEX::Prng Namespace</seealso>
/// <seealso cref="CEX::Prng::IRandom ">CEX::Prng IRandom Interface</seealso>
...
/// <item><description>Prng can be any of the <see cref="CEX::Enumeration::Prngs"/> generators.</description></item>
...
public:
	...
	/// </summary>
	/// 
	/// <param name="SeedType">The <see cref="SeedGenerators">Seed Generators</see> that supplies the seed material to the hash function</param>
	/// <param name="DigestType">The <see cref="Digests">Digest</see> type used to post-process the pseudo random seed material</param>
	KeyGenerator(SeedGenerators SeedType = SeedGenerators::CSPRsg, Digests DigestType = Digests::SHA512)
		:
		_ctrLength(0),
		_ctrVector(0),
		_dgtType(DigestType),
	...
	/// </summary>
	/// 
	/// <param name="PrngType">The <see cref="Prngs">Prng</see> that supplies the seed material to the hash function</param>
	/// <param name="DigestType">The <see cref="Digests">Digest</see> type used to post-process the pseudo random seed material</param>
	...
	KeyGenerator(const SeedGenerators SeedType, const Digests DigestType, const std::vector<byte> &Counter)
		:
		...
		_rngType(SeedType)
	{
		if (Counter.size() > 32 || (Counter.size() < 4 && Counter.size() != 0))
			throw CryptoGeneratorException("KeyGenerator:Ctor", "The counter size must be either 0, or between 4 and 32 bytes!");
	...
	KeyParams* GetKeyParams(const unsigned int KeySize, const unsigned int IVSize = 0, unsigned int IKMSize = 0);
...
protected:
	...
	IDigest* GetDigestEngine(Digests DigsetType);
	ISeed* GetSeedEngine(SeedGenerators SeedType);
